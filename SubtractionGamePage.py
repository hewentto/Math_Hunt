...
def Game(window_surface):
    ...
    #duck
    subDucks = [Duck.duck(i,window_surface) for i in dic.subdictanswer]
    ...
    while is_running:
        ...
        for k in range(len(subDucks)):
            subDucks[k].draw()
            subDucks[k].mover()
        for answer in dic.subdictanswer:
            active_answer = dic.subdictanswer.get(answer)
